...
import { generateEncryptedDownloadUrl, generateShareWhatsAppPdfMessage } from '../../lib/reportSecurity';
...
export const AdminDashboardPage: React.FC<AdminDashboardPageProps> = ({
  ...
  const [sharePdfToken, setSharePdfToken] = useState<TokenRecord | null>(null);
  ...
  return (
    <div className="flex-1 bg-slate-50/50 pb-16">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-3xl w-full max-h-[92vh] overflow-y-auto p-6 space-y-6">
            ...
            <div className="flex items-start justify-between pb-4 border-b border-slate-100">
              ...
              <div className="flex items-center gap-2">
                ...
                {/* Share Encrypted Download Link to Client */}
                <button
                  onClick={() => setSharePdfToken(detailToken)}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-purple-700 hover:bg-purple-800 text-white text-xs font-semibold transition shadow-xs"
                  title="Bagikan Tautan Unduh Terenkripsi ke Klien"
                >
                  <Share2 className="w-3.5 h-3.5" />
                  <span>Bagikan Link PDF</span>
                </button>
            ...
            <div className="pt-2 flex flex-wrap justify-between items-center gap-2 border-t border-slate-100">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  onClick={() => setSharePdfToken(detailToken)}
                  className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-purple-700 hover:bg-purple-800 text-white text-xs font-semibold transition shadow-xs"
                >
                  <Share2 className="w-3.5 h-3.5" />
                  <span>Bagikan Link Download PDF</span>
                </button>
              </div>
      ...
      {/* ========================================================================= */}
      {/* MODAL: SHARE ENCRYPTED PDF DOWNLOAD LINK KE KLIEN */}
      {/* ========================================================================= */}
      {sharePdfToken && currentSession && (
          ...
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-slate-200 p-6 space-y-5">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-bold text-slate-800 text-sm">Bagikan Hasil PDF ke Klien</h3>
                  <p className="text-xs text-slate-500">Tautan Terenkripsi & Verifikasi Token</p>
                </div>
              </div>
              <button
                onClick={() => setSharePdfToken(null)}
                className="text-slate-400 hover:text-slate-600 p-1"
              >
                <X className="w-5 h-5" />
              </button>
            ...
            <div className="p-3.5 bg-purple-50/80 rounded-xl border border-purple-200/80 space-y-2 text-xs">
              <div className="flex items-center justify-between font-semibold text-purple-900">
                <span>Nama Klien:</span>
                <span className="text-slate-800">{sharePdfToken.client_biodata?.fullName || 'Peserta'}</span>
              </div>
              <div className="flex items-center justify-between font-semibold text-purple-900">
                <span>Kunci Token Verifikasi:</span>
                <span className="font-mono bg-white px-2 py-0.5 rounded border border-purple-200 text-purple-800 tracking-wider">
                  {sharePdfToken.token}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-700">
                Tautan Download PDF (Terenkripsi):
              </label>
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  readOnly
                  value={generateEncryptedDownloadUrl(sharePdfToken.token, currentSession.id)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs font-mono text-slate-600 select-all focus:outline-none truncate"
                />
                <button
                  onClick={() => {
                    const url = generateEncryptedDownloadUrl(sharePdfToken.token, currentSession.id);
            ...
            <div className="text-[11px] text-slate-500 bg-slate-50 p-3 rounded-xl border border-slate-200/80 leading-relaxed">
              <strong className="text-slate-700">🔒 Perlindungan Medis Klinis:</strong> Tautan ini aman dan tidak mengizinkan unduhan langsung tanpa verifikasi identitas. Klien wajib menginput token <span className="font-mono font-bold text-slate-800">{sharePdfToken.token}</span> saat membuka tautan sebelum dokumen PDF dibuat.
            </div>
            ...
            <div className="pt-2 flex flex-col gap-2">
              <a
                href={`https://api.whatsapp.com/send?text=${encodeURIComponent(
                  generateShareWhatsAppPdfMessage(
                    sharePdfToken.client_biodata?.fullName || 'Klien',
                    generateEncryptedDownloadUrl(sharePdfToken.token, currentSession.id)
                  )
                )}`}
                target="_blank"
                rel="noopener noreferrer"
                className="w-full inline-flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs transition shadow-xs"
              >
                ...
                <span>Kirim Format Pesan ke WhatsApp Klien</span>
              </a>
